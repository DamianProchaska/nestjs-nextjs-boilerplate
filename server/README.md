...
GraphQL Playground for the NestJS Server is available here: http://localhost:8080/graphql
...
[RESTful API](http://localhost:8080/api) documentation available with Swagger.
...
```bash
docker run -d -t -p 8080:8080 --env-file .env nest-prisma-server
```

Now open up [localhost:8080](http://localhost:8080) to verify that your nest server is running.
