...
import { useRequestPasswordlessSignup, useVerifyOtp } from '@/hooks/auth';
...
function EmailForm({ onSuccess }: { onSuccess: (email: string) => void }) {
  ...
  const requestPasswordlessSignup = useRequestPasswordlessSignup();

  const onSubmit = async (values: EmailFormValues) => {
    try {
      await requestPasswordlessSignup.mutateAsync({ email: values.email });
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  ...
                  {...field}
                  disabled={requestPasswordlessSignup.isLoading}
                />
              </FormControl>
              <FormMessage />
            </FormItem>
        ...
        <Button
          ...
          disabled={requestPasswordlessSignup.isLoading}
        >
          {requestPasswordlessSignup.isLoading ? 'Sending...' : 'Continue with Email'}
        </Button>
      </form>
    </Form>
  );
